...
def db_catalogue(num_films):
    try:
        # Conectamos con la base de datos
        db_conn = None
        db_conn = db_engine.connect()

        # Seleccionamos las primeras "num_films" peliculas de la base de datos
        db_films = select([db_table_movies]).limit(num_films)
        db_result = db_conn.execute(db_films)
